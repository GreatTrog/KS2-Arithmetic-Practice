...
const getLongDivisionExplanation = (q: Question): string[] => {
    ...
    return [
        `**Set up the division.** We are calculating **${dividend} ÷ ${divisor}**. We use a method of repeated subtraction or 'chunking'. It can be helpful to write out some multiples of ${divisor} first (e.g., 1x, 2x, 5x, 10x).`,
        `**First group.** How many times does **${divisor}** go into the first part of **${dividend}**? Let's try a multiple we know. For example, ${divisor} × 10 = ${parseInt(divisor)*10}.`,
        `**Subtract and bring down.** Subtract this 'chunk' from the dividend. Then, bring down the next digit from the dividend to form a new number.`,
        `**Repeat.** Repeat the process: find how many times **${divisor}** goes into your new number, subtract that chunk, and keep track of your groups. Add up the number of 'groups' you used to get the final answer: **${q.answer}**.`
    ];
};
